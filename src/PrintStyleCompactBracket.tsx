...
const TeamSlot: React.FC<TeamSlotProps> = ({ team, isWinner, onClick, reversed = false }) => {
  if (!team) {
    return (
      <div className={`h-7 bg-gray-50 border border-gray-200 rounded-md flex items-center px-2 text-gray-400 text-xs ${
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <span className={`font-bold text-[10px] w-4 h-4 rounded-full bg-gray-200 flex items-center justify-center ${
        reversed ? 'ml-1' : 'mr-1'
      }`}>
        {team.seed}
      </span>
...
const RegionColumn: React.FC<RegionColumnProps> = ({ 
  ...
  return (
    <div className={`w-32 ${reversed ? 'ml-1' : 'mr-1'}`}>
      {roundLabel && (
        <div className="text-xs font-medium text-center text-gray-700 mb-1">{roundLabel}</div>
      )}
  ...
  return (
    <div className="flex flex-col items-center mx-2">
      <div className="flex justify-center items-start mb-3 w-full max-w-xs">
        {/* Championship */}
        <div className="border-2 border-yellow-500 rounded-lg bg-yellow-50 p-2 w-full">
          ...
          {championshipMatchup.winner && (
            <div className="mt-2 pt-1 border-t border-yellow-400">
              <p className="text-center text-xs font-bold text-yellow-800 mb-0.5">CHAMPION</p>
              <div className="bg-green-100 border border-green-500 rounded-md p-1 mx-auto">
                <div className="flex items-center justify-center">
                  ...
                  <span className="font-bold text-xs">{championshipMatchup.winner.name}</span>
                </div>
              </div>
            </div>
          )}
    ...
    <div className="bg-white p-4 rounded-lg shadow-md mb-6 overflow-x-auto">
      ...
      <div className="flex justify-center">
        ...
        <div className="flex flex-col justify-center min-w-32">
          <div className="text-xs font-bold text-center text-red-800 mb-1">Final Four</div>
          <FinalFour 
            bracketData={bracketData}
            onTeamSelect={onTeamSelect}
          />
      ...
      <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs">
        <h3 className="font-bold text-blue-800 mb-1">How to Complete Your Bracket</h3>
